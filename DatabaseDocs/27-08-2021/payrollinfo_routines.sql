...
proc_label:BEGIN
    ...
    WHILE icnt<=tot_cnt DO
        ...
        SET p_hra=(SELECT earn_ded_amount 
			...
			INNER JOIN mst_earn_ded_components medc ON tess.earn_ded_id=medc.earn_ded_id
			INNER JOIN mst_earn_ded_tag medt ON medc.earn_ded_tag_id=medt.earn_ded_tag_id
			WHERE tess.emp_id=p_emp_id AND medt.earn_ded_tag_name='HRA');
        ...
        SET p_taxable_income= p_total_earn_amount - IFNULL(p_total_investment_amount,0);
        ...
        BEGIN
			...
			IF(ifnull(p_existing_tax_rate,0)=0) THEN
				...
				SET p_tax_amount=CEILING((p_existing_tax_rate * p_taxable_income)/(100 *12));
			END IF;
...
-- Dump completed on 2021-08-27 21:04:55
