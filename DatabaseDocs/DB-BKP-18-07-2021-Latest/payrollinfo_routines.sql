...
proc_label:BEGIN
   ...
   INSERT INTO trn_monthly_emp_salary_details 
    ...
    SET tsd1.pay_sheet_columns=(
		SELECT JSON_ARRAYAGG(JSON_OBJECT('earn_ded_id', medc.earn_ded_id,'earn_ded_type',medc.earn_ded_type,'earn_ded_code',medc.earn_ded_code)) AS EarnDedComponents
		from payrollinfo.mst_earn_ded_components medc)
    WHERE tsd1.month=p_month AND tsd1.year=p_year;

	UPDATE trn_monthly_emp_salary_summary tsd1
    SET tsd1.emp_detail_values=(
		SELECT JSON_ARRAYAGG(JSON_OBJECT('emp_name',
				CONCAT(me.emp_first_name,
...
-- Dump completed on 2021-07-18 17:52:58
