...
proc_label:BEGIN
    ...
    DECLARE p_monthly_tax_amount DOUBLE;
    DECLARE p_itax_id INT;
    DECLARE p_itax_slno INT;
    DECLARE p_total_itax_slno INT;
    DECLARE p_existing_tax_rate INT;
    DECLARE p_std_deduction DOUBLE;
    DECLARE p_balance DOUBLE;
    DECLARE p_amt DOUBLE;
    DECLARE p_education_cess DOUBLE;
    ...
    WHILE icnt<=tot_cnt DO
		...
		SET p_total_earn_amount=(SELECT 12 * SUM(tmess.earn_ded_amount) 
        ...
        SET p_total_pf=(SELECT 12 * earn_ded_amount 
			FROM trn_emp_salary_structure tess
			INNER JOIN mst_earn_ded_components medc ON tess.earn_ded_id=medc.earn_ded_id
			INNER JOIN mst_earn_ded_tag medt ON medc.earn_ded_tag_id=medt.earn_ded_tag_id
			WHERE tess.emp_id=p_emp_id AND medt.earn_ded_tag_name='PF');
		       
        SET p_total_ptax=(SELECT 12 * earn_ded_amount 
			FROM trn_emp_salary_structure tess
			INNER JOIN mst_earn_ded_components medc ON tess.earn_ded_id=medc.earn_ded_id
			INNER JOIN mst_earn_ded_tag medt ON medc.earn_ded_tag_id=medt.earn_ded_tag_id
			WHERE tess.emp_id=p_emp_id AND medt.earn_ded_tag_name='PTAX');
        ...
        SET p_total_ded_amount=(SELECT 12 * SUM(tmess.earn_ded_amount) 
				FROM trn_emp_salary_structure tmess
				INNER JOIN mst_earn_ded_components medc
				ON tmess.earn_ded_id=medc.earn_ded_id
        ...
        SET p_investment_amount=(SELECT teyts.investment_amount FROM trn_emp_yearly_tax_savings teyts
					WHERE teyts.emp_id=p_emp_id AND teyts.fin_year_id=p_fin_year_id);
		
        SET p_std_deduction=50000;
        SET p_education_cess=4193;
        SET p_taxable_income= (p_total_earn_amount) - (IFNULL(p_total_ptax,0) + p_std_deduction + IFNULL(p_investment_amount,0));
        
		IF p_taxable_income > 0 THEN
        BEGIN
			SET p_total_itax_slno=(SELECT itax_slab_id FROM trn_itax_slab 
				WHERE fin_year_id=p_fin_year_id AND p_taxable_income BETWEEN itax_start_range AND itax_end_range);
            SET p_itax_slno=1;
            SET p_balance=0;
            SET p_tax_amount=0;
  			WHILE p_itax_slno<=p_total_itax_slno DO
				IF p_itax_slno=1 THEN
                BEGIN
					SET p_balance=(SELECT p_taxable_income - itax_end_range from trn_itax_slab WHERE itax_slab_id=p_itax_slno and itax_rate_existing=0);
                    SET p_tax_amount=0;
				END;
                ELSEIF p_itax_slno!=p_total_itax_slno THEN
                BEGIN
					SET p_balance=(SELECT p_balance - (itax_start_range -1) from trn_itax_slab WHERE itax_slab_id=p_itax_slno);
                    SET p_amt=(SELECT (itax_rate_existing * (itax_start_range -1))/100 from trn_itax_slab WHERE itax_slab_id=p_itax_slno );
                    SET p_tax_amount=p_tax_amount + p_amt;
                END;
                ELSE
                BEGIN
					SET p_amt=(SELECT (itax_rate_existing * p_balance)/100 from trn_itax_slab WHERE itax_slab_id=p_itax_slno );
                    SET p_tax_amount=p_tax_amount + p_amt;
                END;
                END IF;
				SET p_itax_slno=p_itax_slno + 1;
			END WHILE;
            
            SET p_tax_amount=p_tax_amount + p_education_cess ;
            SET p_monthly_tax_amount=ROUND(p_tax_amount/12,0);
...
proc_label:BEGIN
    ...
    DECLARE lock_month INT;
    DECLARE lock_year INT;
    DECLARE p_project_start_date DATE;
    DECLARE p_no_hours_worked DECIMAL(5,2);
    ...
    SET p_project_start_date=(SELECT PROJECT_START_DATE FROM MST_COMPANY);
    IF p_start_date<p_project_start_date THEN
		BEGIN
			SET p_return_code=5;
			SET p_return_message=CONCAT(CONCAT(p_return_code,'-'),'Attendance cannot be taken before project start date');
			LEAVE proc_label;
		END;
    END IF;
    ...
    BEGIN
		IF p_month=1 THEN
			BEGIN
				SET lock_month=12;
				SET lock_year=p_year-1;
			END;
		ELSE
			BEGIN
				SET lock_month=p_month - 1;
				SET lock_year=p_year;
			END;
		END IF;
		IF NOT EXISTS (SELECT * FROM trn_monthly_salary_lock 
			WHERE month=lock_month AND year=lock_year AND salary_lock='Y') THEN
			BEGIN
				SET p_return_code=5;
				SET p_return_message=CONCAT(CONCAT(p_return_code,'-'),'Attendance cannot be taken as salary is not locked in previous month');
				LEAVE proc_label;
			END;
		END IF;
    END;
    ...
    IF tot_cnt=0 THEN
		SET p_return_code=5;
        SET p_return_message='Employee salary structure has not been generated hence attendance generation is not possible';
        SET p_return_message=CONCAT(CONCAT(p_return_code,'-'),p_return_message);
        LEAVE proc_label;
    END IF;
    ...
    WHILE icnt<=tot_cnt DO
		...
		SET p_no_hours_worked=8;
		SET p_day=1;
		WHILE p_day <= p_end_day DO 
			IF p_day=1 THEN
				SET p_attendance_date=p_start_date;
			else
				SET p_attendance_date=date_add(p_attendance_date,INTERVAL 1 DAY);
			END IF;
            INSERT INTO trn_emp_attendance (month,year,emp_id,attendance_date,attendance_type_code,no_hours_worked)
			VALUES(p_month,p_year,p_emp_id,p_attendance_date,'P',p_no_hours_worked);
			SET  p_day = p_day + 1;
		 END WHILE;
    ...
    DROP TEMPORARY TABLE temp_mst_employee;
    SET p_return_code=0;
    SET p_return_message=CONCAT('Attendance has been generated for ',DATE_FORMAT(p_start_date, "%M %Y"));
    SET p_return_message=CONCAT(CONCAT(p_return_code,'-'),p_return_message);
    COMMIT;
END ;;
DELIMITER ;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `ManageSingleEmpAttendance` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `ManageSingleEmpAttendance`(IN p_month INT,IN p_year INT,IN p_emp_id INT,IN p_attendance_date DATE,IN p_no_hours_worked DECIMAL(4,2),OUT p_return_message VARCHAR(500))
proc_label:BEGIN
	DECLARE p_project_start_date DATE;
	DECLARE lock_month INT;
    DECLARE lock_year INT;
    ...
    SET p_project_start_date=(SELECT PROJECT_START_DATE FROM MST_COMPANY);
    IF p_attendance_date<p_project_start_date THEN
		BEGIN
			SET p_return_message=CONCAT(CONCAT(5,'-'),'Attendance cannot be taken before project start date');
			LEAVE proc_label;
		END;
    END IF;
	
	IF EXISTS(SELECT 1 FROM mst_holiday WHERE holiday_date=p_attendance_date) THEN
		SET p_return_message=CONCAT(CONCAT(5,'-'),'This day is holiday so Attendance cannot be taken');
        Leave proc_label;
    END IF;
    ...
    BEGIN
		...
		IF NOT EXISTS (SELECT * FROM trn_monthly_salary_lock 
			WHERE month=lock_month AND year=lock_year AND salary_lock='Y') THEN
			BEGIN
				SET p_return_message=CONCAT(CONCAT(5,'-'),'Attendance cannot be taken as salary is not locked in previous month');
				LEAVE proc_label;
			END;
		END IF;
    END;
    ...
    IF EXISTS(SELECT 1 FROM trn_emp_leave 
			WHERE emp_id=p_emp_id AND is_approved='Y' AND
            p_attendance_date BETWEEN from_date AND to_date) THEN
		SET p_return_message=CONCAT(CONCAT(5,'-'),'Leave has been granted hence Attendance cannot be taken');
        Leave proc_label;
    END IF;
	IF EXISTS(SELECT 1 FROM trn_emp_attendance 
				WHERE month=p_month AND year=p_year
                AND emp_id=p_emp_id AND attendance_date=p_attendance_date) THEN
		UPDATE trn_emp_attendance
        SET no_hours_worked=p_no_hours_worked
        WHERE month=p_month AND year=p_year
		AND emp_id=p_emp_id AND attendance_date=p_attendance_date;
	ELSE
		INSERT INTO trn_emp_attendance(month,year,emp_id,attendance_date,no_hours_worked)
        VALUES(p_month,p_year,p_emp_id,p_attendance_date,p_no_hours_worked);
	END IF;
    SET p_return_message=CONCAT(CONCAT(0,'-'),'Attendance Updated Successfully');
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `PrintEmployeeLeave` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `PrintEmployeeLeave`(IN p_from_date date,IN p_to_date date)
BEGIN
	SELECT tea.emp_id,me.emp_code,
	CONCAT(me.emp_first_name,' ',
	CASE WHEN LENGTH(me.emp_middle_name)>0 THEN CONCAT(me.emp_middle_name,' ') 
	ELSE me.emp_middle_name END,me.emp_last_name) AS emp_name,
	tea.attendance_date,mlt.leave_type_name
	FROM payrollinfo.trn_emp_attendance tea
	INNER JOIN mst_employee me ON tea.emp_id=me.emp_id
	INNER JOIN mst_leave_type mlt ON tea.leave_type_code=mlt.leave_type_code
	WHERE tea.attendance_date>=p_from_date AND tea.attendance_date<=p_to_date
	AND tea.attendance_type_code='L'
	ORDER BY me.emp_code,tea.attendance_date;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
-- Dump completed on 2021-09-10 17:18:25
